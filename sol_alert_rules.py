...
from bot_config import logger # Impor logger terpusat
# === TAMBAHKAN ATAU PASTIKAN BARIS IMPOR INI ADA ===
from utils import escape_markdown_v2, get_val 
# === AKHIR TAMBAHAN ===

# ... sisa fungsi generate_sol_alerts Anda ...

# Contoh penggunaan di dalam generate_sol_alerts (seperti yang sudah ada):
# def generate_sol_alerts(data_frames_by_timeframe: dict, pair_name_for_alert: str) -> str:
#     alerts = []
#     escaped_pair_name = escape_markdown_v2(pair_name_for_alert) # Ini sekarang akan berfungsi
#     ...

def generate_sol_alerts(data_frames_by_timeframe: dict, pair_name_for_alert: str) -> str:
    # (Salin implementasi generate_sol_alerts yang sudah diperbaiki ke sini)
    alerts = []
    escaped_pair_name = escape_markdown_v2(pair_name_for_alert)
    logger.info(f"Memulai generate_sol_alerts untuk {pair_name_for_alert} dengan timeframe yang tersedia: {list(data_frames_by_timeframe.keys())}")
    ...
    # Contoh Aturan 1 (menggunakan 5m dan 1H)
    if last_5m is not None and last_1H is not None:
        ...
        if rsi_5m < 25 and close_5m < ema20_5m * 0.995: 
             if rsi_1H < 35 and close_1H > ema50_1H and (is_bullish_eng_5m or is_hammer_5m):
                alerts.append(f"🔥 *SOL Alert BUY Potensial (Agresif)*:\n  RSI 5m ({rsi_5m:.1f}) & 1H ({rsi_1H:.1f}) rendah. Pola Bullish 5m. 1H di atas support EMA50.\n  Harga 5m: {get_val(last_5m, 'close')}")

    # Contoh Aturan 2: MACD Cross Bullish di 5m + Konfirmasi Tren Naik di 1H
    ...
    # Contoh Aturan 3 (menggunakan 15m, hanya aktif jika data 15m tersedia)
    if last_15m is not None and df_15m is not None and len(df_15m) >= 21 and \
       last_1H is not None:
